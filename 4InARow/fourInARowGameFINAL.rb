...
def loadSteps2()
  ...
  file = File.open(filename)
  file_data = file.read
  # eval(file_data) will execute the data from file as ruby code.
  return eval(file_data)
end
...
def saveGame(board)
  ...
  #File.open - open the file "w" - open the file for writing "#{Time.now}" - writes the current time in the file f.write - writes the current time in the file f - file handle
  #The code opens the file in write mode, writes the current time in the file and then closes the file. t is a way of processing a file such as creating a new file, reading content in a file, writing content to a file, appending content
  #{ |f| f.write "#{Time.now}" } - block of code
